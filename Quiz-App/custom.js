...
category.forEach(function(element){
    ...
    element.addEventListener('click', function(e){
        ...
        let fetchRes =  fetch(
            `https://opentdb.com/api.php?amount=10&category=${getCategoryValue}
        ...
        res.json()).then( response =>{
            ...
            totalResults.map((item, key) => {
                ...
                itemHtml = `${(key == 0) ? '<div class="question-wrap active">' : '<div class="question-wrap">'}
                <div class="question-box"><div class="totalQuestionCurrent">
                <span class="currentQuesNo">${key + 1}</span>/10
                </div>
                <h3 class="currentQues">${item.question}</h3>
                </div>
                <div class="quesOptions">
                <ul class="questionList">${optionRandom[0]}${optionRandom[1]}${optionRandom[2]}${optionRandom[3]}</ul>
                </div>
                </div>`;
